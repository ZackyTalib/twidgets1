...
export default class ChatFeed {
    ...
    setupS1(){
        ...
        const socket = io("https://chat.s1live.com/chat");
        ...
        socket.on("message", (data) => {
            ...
            this.processS1Chat({
                ...
                name: data.name,
                message: data.msg,
                pfp: data.logo == "null" ? "https://cdn.s1live.com/clubs/custom-logos/GEN.png" : "https://media.api-sports.io/football/teams/" + data.logo + ".png",
                userStatus: "normal",
                platform: "s1live"
            })
        });
    ...
    processS1Chat(s1ChatRaw){
        if(!this.chatOn){
            return;
        }

        this.auth.analytics.logEvent('chatLoad', {type: "MessageChat", userStatus: "normal", platform: "s1live"});
        let s1Chat = s1ChatRaw;
        s1Chat.template = this.app.appSettings.settingTemplates.SuperSticker;
        console.log(s1Chat);
        this.chats.push(s1Chat);
        this.chatCallback.call(this.app, null, s1Chat);
    }
}
