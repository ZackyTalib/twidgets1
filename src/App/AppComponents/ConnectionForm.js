...
export default class ConnectionForm {
    ...
    async showPopup(callback){
        ...
        document.getElementById("connectButtonOk").addEventListener('click', () => {
            ...
            let s1Value = document.querySelector("#s1Tab select").value;

            if(!this.validURL(youtubeValue) && !this.validURL(facebookValue) && !this.validURL(twitchValue) && !s1Value){
                ...
                document.querySelector("#twitchTab input").classList.add("swal2-inputerror");
                document.querySelector("#s1Tab select").classList.add("swal2-inputerror");
            } else {
                Swal.close();
                callback(true, {
